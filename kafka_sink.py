...
class KafkaSink:
    def __init__(self, brokers, topic, *, queue_size=10000, extra_conf=None):
        ...
        self._conf = {
            ...
            "enable.idempotence": True,      # 幂等，配合下游按 msg_id 去重
            "acks": "all",
    ...
    def _run(self):
        ...
        while not self._stop.is_set() or not self._q.empty():
            ...
            if producer is None:
                print(f"[KafkaSink:dryrun] {payload}", flush=True)
                continue

            try:
                producer.produce(
                    self.topic,
                    key=str(payload.get("chat", "")).encode("utf-8"),
